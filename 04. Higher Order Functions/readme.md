...
> [!NOTE]
> Стрелката `->` се нарича типов конструктор, който е в частност двуместен.
  Това означава, че приема два произволно сложни типа `a` и `b` и създава нов тип
  `a -> b`, който представя функция с домейн `a` и кодомейн `b`. Тъй като `->` е типов конструктор,
  т.е. сама по себе си е функция (над типове), можем да я запишем и префиксно по следния начин:
  `(->) a b`, което е напълно същото като инфиксния запис `a -> b`.
...
Друг понякога полезен оператор е `(&) :: a -> (a -> b) -> b`,
който е просто обърнатата версия на `$` (и е лявоасоциативен):
...
В конкретни ситуации е една идея по-четимо неговото използване (вместо `$`),
тъй като четем изпълнението на операциите отляво на дясно (което по-добре
илюстрира последователността на действията):

```hs
-- обикновено бихме използвали по-силна абстракция
-- вместо прост типов синоним, но все още не сме го учили
type Stack a = [a]
...
където последователно добавяме 1, 2 и 3 в първоначално празен стек.
Ако бяхме използвали `$`, бихме написали:
...
```
В действителност в Haskell вече има такава функция liftA2, която също би работила в случая, но имплементацията ѝ е сравнително по-сложна (макар и аналогична) на това, което написахме по-горе.
